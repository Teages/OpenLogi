...
/// compare-and-restore a durable journal record and returns immediately. A
/// normal stop restores every diverted control before returning; transport
...
async fn run_capture_session_on(
    ...
    let listener = add_control_listener(&chan, &armed, &accum, device_index, &sink);
...
fn add_control_listener(
    ...
    let thumb_resolution = armed
        .thumb
        .as_ref()
        .map_or(WheelResolution::UNKNOWN, |(_, resolution)| *resolution);
    ...
    loop {
        tokio::select! {
            _ = &mut shutdown => {
                ...
                // be obsolete.
                return (stop_for_current_publication(context.registry, context.shared), None);
            }
